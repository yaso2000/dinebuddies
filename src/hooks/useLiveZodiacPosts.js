...
/**
 * Live "Guess my sign?" cards for the stories rail + deck. Only OTHER people's
 * live cards (never the viewer's own — they can't guess their own). 24h TTL;
 * expired dropped client-side. Followed owners first, then newest.
 */
export function useLiveZodiacPosts({ enabled = true, cap = 30 } = {}) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    const q = query(collection(db, 'zodiac_posts'), where('status', '==', 'live'), limit(60));
    return onSnapshot(
      q,
