...
/**
 * Swipe-left row with a single Delete action (inbox activity, etc.).
 * `direction: ltr` keeps swipe geometry correct in RTL UI.
 */
export default function DeleteSwipeRow({
    rowId,
    isOpen,
    onOpenChange,
    onDelete,
    ...
    const onPointerDown = (e) => {
        if (e.target.closest('.inbox-activity-row__dismiss')) return;
    ...
    return (
        <div
            className={`delete-swipe${isOpen ? ' delete-swipe--open' : ''}${
                dragging ? ' delete-swipe--dragging' : ''
            } ${className}`.trim()}
            data-swipe-id={rowId}
        >
            <div className="delete-swipe__actions" aria-hidden={!actionsVisible}>
                <button
                    type="button"
                    className="delete-swipe__action"
            ...
            >
                <div className="delete-swipe__front" onClick={handleFrontClick}>
                    {children}
                </div>
            </div>
        </div>
