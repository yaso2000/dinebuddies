...
const PostCard = ({ post, showInChat = false, defaultExpandComments = false }) => {
  ...
  const toggleReplyThread = (parentId) => {
    setExpandedReplyIds((prev) => {
      ...
      if (next.has(parentId)) next.delete(parentId);else
      next.add(parentId);
      return next;
    });
  };
  ...
  let authorAvatar = post.businessLogoUrl || resolveFeedAuthorAvatar(post, userData, userProfile, authorId, currentUser?.uid);
  ...
  if (userData) {
    ...
    authorAvatar = resolveFeedAuthorAvatar(post, userData, null, authorId, null);
  ...
  if (currentUser?.uid === authorId && userProfile) {
    ...
    authorAvatar = resolveFeedAuthorAvatar(post, userData, userProfile, authorId, currentUser?.uid);
    authorHandle = userProfile.username || userProfile.handle || authorHandle;
  } else if (!userData) {
    authorAvatar = resolveFeedAuthorAvatar(post, null, null, authorId, currentUser?.uid);
  }
  ...
  displayPost.businessLogoUrl ||
  resolveFeedAuthorAvatar(displayPost, userData, userProfile, displayAuthorId, currentUser?.uid);
  ...
  if (currentUser?.uid === displayAuthorId && userProfile && !isRepost) {
    ...
    displayAuthorAvatar = resolveFeedAuthorAvatar(displayPost, userData, userProfile, displayAuthorId, currentUser?.uid);
  }
  ...
  return (
    <div
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '8px', padding: '16px 16px 0 16px' }}>
                    <BiRepost size={16} />
                    <AppText as="span" style={{ fontWeight: 'bold' }}>{authorName} reposted</AppText>
                </div>
      }
          ...
          onClick={(e) => {
            e.stopPropagation();
            if (displayPost.partnerId) navigate(`/business/${displayPost.partnerId}`);else
            if (displayAuthorId) navigate(`/profile/${displayAuthorId}`);
          }} />


                <div style={{ display: 'flex', flexDirection: 'column', lineHeight: '1.2', flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '4px' }}>
                        <AppText as="span" className="post-user-name" style={{ fontWeight: 'bold', color: 'var(--text-main)', fontSize: '0.85rem', lineHeight: '1.2', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                            {displayAuthorName}
                        </AppText>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.80rem', color: 'var(--text-muted)', overflow: 'hidden' }}>
                        <AppText as="span" className="post-user-handle" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {displayAuthorHandle}
                        </AppText>
                        <AppText as="span" style={{ flexShrink: 0 }}>·</AppText>
                        <AppText as="span" className="post-time" style={{ flexShrink: 0 }}>
        ...
        <div style={{ borderRadius: 16, overflow: 'hidden', border: '1px solid var(--border-color)', marginBottom: 8, background: 'var(--bg-card)' }}>
            ...
            <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.1)' }}>
                        ...
                        <div style={{ padding: 16 }}>
                            ...
                            <AppText as="h4" style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--text-main)', margin: '0 0 8px 0', lineHeight: 1.3 }}>
                                {displayPost.eventDetails.title}
                            </AppText>
                            <AppText as="p" style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', margin: '0 0 16px 0', lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.85rem', color: 'var(--text-main)' }}>
                                        ...
                                        <AppText as="span">{displayPost.eventDetails.location}</AppText>
                                    </div>
              }
                            </div>
                ...
                onError={(e) => {
                  ...
                  e.currentTarget.src = `https://img.youtube.com/vi/${displayPost.mediaUrl}/hqdefault.jpg`;
                                            ...
                                            <div className="post-media-preview__play">
                                                <AppText as="span" className="post-media-preview__play-icon">
        ...
        <div
          ...
          onClick={(e) => {
            if (commentsRaw.length > 0) {
              ...
              setShowComments(true);
                        ...
                        <div className="post-engagement-row__left">
                            {localLikes.length > 0 ?
            <AppText as="span" className="post-engagement-row__stat post-engagement-row__stat--likes">
            ...
            onClick={handleLike}>
                        ...
                        <AppText as="span">{t('like', 'Like')}</AppText>
                        {localLikes.length > 0 ?
            ...
            onClick={(e) => {
              ...
              setShowShare(true);
                        ...
                        <AppText as="span">{t('share', 'Share')}</AppText>
                    </button>
                </div>
        ...
        <div
          ...
          null}
                        <button type="button" className="fb-comments-sort" onClick={(e) => e.stopPropagation()}>
                            ...
                            <AppText as="span" className="fb-comments-sort__chevron" aria-hidden>
                                ▾
                            </AppText>
                        </button>
