...
const PostCard = ({ post, showInChat = false }) => {
    ...
    useEffect(() => {
        ...
        // ALWAYS FETCH to ensure we show the latest profile data (name/avatar changes)
        // This fixes the issue where users might appear as "Guest" if the post has stale data.
    ...
    let authorName = post.partnerName || post.author?.name || post.userName || post.user_name || post.displayName || post.name;
