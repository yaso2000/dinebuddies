  ...
  return (
    <div
      ...
      <div style={{ padding: '8px 12px', display: 'flex', alignItems: 'center', gap: '8px', borderBottom: '1px solid var(--border-color)', background: 'var(--bg-elevated)' }}>
                    ...
                    <AppText as="span" style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-main)' }}>{data.authorName || 'User'}</AppText>
                </div>
      }
        ...
        <div style={{
          ...
          padding: '2px 6px', borderRadius: '4px', fontSize: '0.7rem', fontWeight: 600, textTransform: 'uppercase'
            ...
            <div style={{ padding: '10px 12px', background: 'var(--bg-card)' }}>
                {data.title &&
        <AppText as="h4" style={{ margin: '0 0 4px 0', fontSize: '0.95rem', color: 'var(--text-main)', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                        {data.title}
                    </AppText>
        }
                {data.description &&
        <AppText as="p" style={{ margin: 0, fontSize: '0.8rem', color: 'var(--text-muted)', display: '-webkit-box', WebkitLineClamp: 4, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                        {data.description}
                    </AppText>
        }
