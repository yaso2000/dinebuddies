...
const GoldCrownIcon = ({ size = 36 }) => (
    <svg width={size} height={size} viewBox="0 0 40 40" style={{ animation: 'crownGlow 2.5s ease-in-out infinite', color: '#ffd700' }}>
...
const SilverCrownIcon = ({ size = 36 }) => (
    <svg width={size} height={size} viewBox="0 0 40 40" style={{ animation: 'crownGlow 3s ease-in-out infinite', color: '#c0c0c0' }}>
...
const BronzeCrownIcon = ({ size = 36 }) => (
    <svg width={size} height={size} viewBox="0 0 40 40" style={{ animation: 'crownGlow 3.5s ease-in-out infinite', color: '#cd7f32' }}>
...
const GoldStarIcon = ({ size = 36 }) => (<svg width={size} height={size} viewBox="0 0 40 40" style={{ animation: 'starPulse 2s ease-in-out infinite' }}><defs><linearGradient id="r_gstar" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#fff8b0"/><stop offset="50%" stopColor="#ffd700"/><stop offset="100%" stopColor="#d4af37"/></linearGradient></defs><polygon points="20,2 24.5,14 38,14 27,22 31,35 20,27 9,35 13,22 2,14 15.5,14" fill="url(#r_gstar)" stroke="#b8860b" strokeWidth=".8" /></svg>);
...
const LightningIcon = ({ size = 36 }) => (<svg width={size} height={size} viewBox="0 0 40 40" style={{ animation: 'lightningZap 2.5s ease-in-out infinite' }}><defs><linearGradient id="r_ltng" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#fff8b0"/><stop offset="50%" stopColor="#fde047"/><stop offset="100%" stopColor="#eab308"/></linearGradient></defs><polygon points="20,2 10,22 20,22 18,38 30,16 20,16" fill="url(#r_ltng)" stroke="#ca8a04" strokeWidth=".8" /></svg>);
const PercentIcon = ({ size = 36 }) => (<svg width={size} height={size} viewBox="0 0 40 40" style={{ animation: 'tagBob 2.2s ease-in-out infinite' }}><defs><linearGradient id="r_ptag" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#f87171" /><stop offset="100%" stopColor="#dc2626" /></linearGradient></defs><path d="M6 22 L6 34 L32 34 Q38 34 38 22 L38 6 L22 6 Q6 6 22 Z" fill="url(#r_ptag)" stroke="#b91c1c" strokeWidth="1" /><circle cx="10" cy="10" r="3" fill="#fca5a5" /><text x="22" y="27" fontFamily="system-ui" fontWeight="900" fontSize="15" fill="#fff" textAnchor="middle">%</text></svg>);
const MedalIcon = ({ size = 36 }) => (<svg width={size} height={size} viewBox="0 0 40 40" style={{ animation: 'medalSwing 2s ease-in-out infinite' }}><defs><linearGradient id="r_medal" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#fff7a0" /><stop offset="50%" stopColor="#ffd700" /><stop offset="100%" stopColor="#b8860b" /></linearGradient></defs><rect x="17" y="1" width="6" height="14" rx="1" fill="#6366f1" /><circle cx="20" cy="28" r="11" fill="url(#r_medal)" stroke="#b8860b" strokeWidth="1.5" /><circle cx="20" cy="28" r="8" fill="none" stroke="#fff7a0" strokeWidth=".8" opacity=".6" /><text x="20" y="32" fontFamily="serif" fontWeight="900" fontSize="10" fill="#b8860b" textAnchor="middle">★</text></svg>);
const RibbonIcon = ({ size = 36 }) => (<svg width={size} height={size} viewBox="0 0 40 40" style={{ animation: 'ribbonPulse 2s ease-in-out infinite' }}><defs><linearGradient id="r_rbn" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#c4b5fd"/><stop offset="50%" stopColor="#8b5cf6"/><stop offset="100%" stopColor="#6d28d9"/></linearGradient></defs><circle cx="20" cy="20" r="14" fill="url(#r_rbn)" stroke="#5b21b6" strokeWidth="1.5"/><polygon points="12,28 20,24 28,28 26,38 20,34 14,38" fill="#7c3aed" /><text x="20" y="24" fontFamily="system-ui" fontWeight="900" fontSize="9" fill="#fff" textAnchor="middle">VIP</text></svg>);
const EmeraldIcon = ({ size = 36 }) => (<svg width={size} height={size} viewBox="0 0 40 40" style={{ animation: 'diamondFacet 2.5s ease-in-out infinite' }}><defs><linearGradient id="r_emg" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#6ee7b7"/><stop offset="50%" stopColor="#10b981"/><stop offset="100%" stopColor="#047857"/></linearGradient></defs><polygon points="20,3 36,14 36,28 20,37 4,28 4,14" fill="url(#r_emg)" stroke="#059669" strokeWidth="1" /><polygon points="20,3 36,14 20,20" fill="rgba(255,255,255,.28)" /><polygon points="4,14 20,20 4,28" fill="rgba(0,80,40,.3)" /><polygon points="20,3 4,14 20,20" fill="rgba(255,255,255,.15)" /><polygon points="20,37 36,28 20,20" fill="rgba(0,0,0,.15)" /></svg>);
...
function IdentityEl({ offer }) {
    ...
    if (offer.logoUrl) return <img src={pickSafeDisplayImageUrl(offer.logoUrl)} alt="logo" style={{ width: iz, height: iz, borderRadius: '50%', objectFit: 'cover', border: '2px solid rgba(255,255,255,.3)', boxShadow: '0 2px 8px rgba(0,0,0,.5)' }} onError={e => { e.target.style.display = 'none'; }} />;
...
function T2({ o }) {
    ...
    return <Card3D offer={o}>
        ...
        <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: '70%', overflow: 'hidden', zIndex: 0 }}>{o.imageUrl ? <img src={pickSafeDisplayImageUrl(o.imageUrl)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <ImgPlaceholder />}</div>
...
function T5({ o }) {
    ...
    const ts = (o.titleSize || 13) + 'px', ds = (o.descSize || 10) + 'px';
    const iz = o.iconSize || 28;
    return <Card3D offer={o}><div style={{ width: '22%', flexShrink: 0, height: '100%', overflow: 'hidden' }}>{o.imageUrl ? <img src={pickSafeDisplayImageUrl(o.imageUrl)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <ImgPlaceholder />}</div><div style={{ flex: 1, textAlign: 'center', padding: '0 12px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}><div style={{ fontSize: ts, fontWeight: 700, color: tc, textTransform: 'uppercase', letterSpacing: '1px', lineHeight: 1.2 }}>{o.title || 'QUIET DINNER'}</div><div style={{ width: 30, height: 1, background: dark ? 'rgba(255,255,255,.22)' : 'rgba(0,0,0,.15)', margin: '4px 0' }} /><div style={{ fontSize: ds, color: sc, letterSpacing: '.4px', textTransform: 'uppercase' }}>{o.description || '15% OFF TODAY'}</div></div><div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', paddingRight: 8 }}><IdentityEl offer={{ ...o, iconSize: iz }} /></div></Card3D>;
}

function T6({ o }) {
    const solidBg = o.bgColor?.startsWith('linear') ? '#0a0a0a' : (o.bgColor || '#0a0a0a');
    const ts = (o.titleSize || 13) + 'px', ds = (o.descSize || 10) + 'px';
    return <Card3D offer={o}><div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: '46%', overflow: 'hidden', zIndex: 1 }}>{o.imageUrl ? <img src={pickSafeDisplayImageUrl(o.imageUrl)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: .52 }} /> : <div style={{ width: '100%', height: '100%' }} />}<div style={{ position: 'absolute', inset: 0, background: `linear-gradient(90deg, ${solidBg} 0%, transparent 55%)` }} /></div><div style={{ zIndex: 4, paddingLeft: 14, flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 3 }}><div style={{ display: 'flex', alignItems: 'center', gap: 5 }}><IdentityEl offer={o} /><div style={{ fontSize: ts, fontWeight: 900, color: o.titleColor || '#ffd600', textTransform: 'uppercase', letterSpacing: '.5px', textShadow: '0 0 10px rgba(255,214,0,.55)' }}>{o.title || "DON'T MISS OUT!"}</div></div><div style={{ fontSize: ds, fontWeight: 900, color: o.descColor || '#fff', textTransform: 'uppercase', letterSpacing: '.4px', lineHeight: 1.1, textShadow: '0 2px 8px rgba(0,0,0,.9)' }}>{o.description || 'BUY 1 GET 1 FREE'}</div></div></Card3D>;
}
