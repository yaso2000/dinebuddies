...
import {
  ...
  resolvePrivateTextBackdropTone } from
'./socialCardTextBackdrop';
import './SocialCardTextBackdropTonePicker.css';
import { AppText } from "../../base";

const TONE_LABEL_KEYS = {
  dark: 'social_card_text_backdrop_tone_dark',
  light: 'social_card_text_backdrop_tone_light',
  glass: 'social_card_text_backdrop_tone_glass',
  none: 'social_card_text_backdrop_tone_none'
};
...
function ToneIcon({ toneId }) {
  if (toneId === 'light') {
    return <AppText as="span" className="private-card-text-backdrop-tone__swatch private-card-text-backdrop-tone__swatch--light" aria-hidden />;
  }
  if (toneId === 'dark') {
    return <AppText as="span" className="private-card-text-backdrop-tone__swatch private-card-text-backdrop-tone__swatch--dark" aria-hidden />;
  }
  if (toneId === 'glass') {
    return <AppText as="span" className="private-card-text-backdrop-tone__swatch private-card-text-backdrop-tone__swatch--glass" aria-hidden />;
  }
...
export default function PrivateCardTextBackdropTonePicker({
  ...
  return (
    <div
      ...
      role="group"
      aria-label={t('social_card_text_backdrop_tone_aria', { defaultValue: 'Text panel tone' })}>

            {variant === 'default' ?
      <AppText as="span" className="private-card-text-backdrop-tone__label">
                    {t('social_card_text_backdrop_tone_label', { defaultValue: 'Panel' })}
                </AppText> :
      null}
            {toneOrder.map((id) => {
