...
/**
 * Lightweight animated SVG per category (CSS keyframes in SocialInvitationCardPreview.css).
...
function SvgWrap({ children, categoryId }) {
  return (
    <AppText as="span" className={`private-card-cat-icon private-card-cat-icon--${categoryId}`} aria-hidden="true">
...
export default function SocialCardCategoryIcon({ categoryId = 'social', personalIntent = false }) {
  ...
  switch (id) {
    ...
    case 'friendship':
      return (
        <SvgWrap categoryId={id}>
                    <circle cx="17" cy="19" r="4.5" stroke="currentColor" strokeWidth="2" fill="none" />
                    <circle cx="31" cy="19" r="4.5" stroke="currentColor" strokeWidth="2" fill="none" />
                    <path d="M10 36c0-6 5-10 14-10s14 4 14 10" stroke="currentColor" strokeWidth="2" fill="none" />
                </SvgWrap>);
