...
import { getDatingCardBackgroundOptions, resolveDatingCardBackgroundUrlCandidates } from './datingCardBackgrounds';
import './DatingPreviewRightRail.css';

function TemplateThumb({ optionId, selected, onClick, title }) {
    const candidates = useMemo(() => resolveDatingCardBackgroundUrlCandidates(optionId), [optionId]);
 ...
 */
export default function DatingPreviewRightRail({
    mode,
    cardBackgroundId,
    ...
    const templateOptions = getDatingCardBackgroundOptions();

    const railLabel =
        mode === 'template'
            ? t('private_card_background_label', { defaultValue: 'Card background' })
            : mode === 'upload'
              ? t('dating_rail_upload_label', { defaultValue: 'Your photo' })
              : t('dating_rail_camera_label', { defaultValue: 'Video' });
    ...
    return (
        <div
            className={`dating-preview-rail private-card-bg-picker private-card-bg-picker--beside${mode === 'template' ? ' dating-preview-rail--template-column' : ''}`.trim()}
        >
            <p className="private-card-bg-picker__label private-card-bg-picker__label--beside">{railLabel}</p>
            <div className="private-card-bg-picker__scroll dating-preview-rail__scroll" role="list">
                {mode === 'template' &&
