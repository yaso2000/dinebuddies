...
import {
    fetchDatingEligibilityByUserIds,
    isUserAvailableForDating,
    mergeDatingEligibility,
} from '../../../utils/datingInviteAvailability';

const PRIVATE_MAX_GUESTS = 30;
const DATING_MAX_GUESTS = 1;

/**
 * Pick in-app invitees on the preview step (not on the create form).
 */
export default function PrivateInvitationInviteePanel({
    invitationId,
    mode = 'private',
    invitedFriendIds = [],
    readOnly = false,
    ...
    const maxGuests = mode === 'dating' ? DATING_MAX_GUESTS : PRIVATE_MAX_GUESTS;
    ...
    useEffect(() => {
        ...
        const loadFriends = async () => {
            ...
            try {
                ...
                let list = friends;
                if (mode === 'dating') {
                    const fieldsMap = await fetchDatingEligibilityByUserIds(friends.map((f) => f.id));
                    list = friends
                        .map((f) => mergeDatingEligibility(f, fieldsMap))
                        .filter((f) => isUserAvailableForDating(f));
                }
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                ...
                const addCandidate = (id, data) => {
                    ...
                    if (mode === 'dating') {
                        if (!isUserAvailableForDating(data)) return;
                    } else {
                ...
                users.forEach((u) => {
                    ...
                    addCandidate(u.id, {
                        ...
                        availableForDating: u.availableForDating,
                ...
                if (mode === 'dating' && results.length) {
                    const fieldsMap = await fetchDatingEligibilityByUserIds(results.map((r) => r.id));
                    results = results
                        .map((r) => mergeDatingEligibility(r, fieldsMap))
                        .filter((r) => isUserAvailableForDating(r));
                }
    ...
    const datingListEmptyMessage = useMemo(() => {
        ...
        return t('dating_no_available_invitees', {
            defaultValue:
                'No one you follow is open to date invitations. Follow members who accept DineBuddy Date.',
        });
    ...
    const persistInvitees = useCallback(
        async (nextIds) => {
            ...
            try {
                await updateDoc(doc(db, 'private_invitations', invitationId), {
    ...
    const title =
        mode === 'dating'
            ? step === 'create'
                ? t('dating_pick_date_create', { defaultValue: 'Who are you inviting?' })
    ...
    const emptyHint =
        mode === 'dating'
            ? step === 'create'
                ? t('dating_pick_date_create_hint', {
                      defaultValue:
                          'Only members who accept date invitations appear here (consumer accounts only).',
                  })
                : t('dating_pick_date_preview', 'Choose who you are inviting below (optional for external link).')
            : t('private_no_in_app_invitees', 'No in-app guests yet — you can still share externally after sending.');

    return (
        <section className={`private-invitee-panel ${className}`.trim()}>
            <div className="private-invitee-panel__head">
                <h3 className="private-invitee-panel__title">
                    <FaUserFriends aria-hidden />
                    {title}
                </h3>
                {!readOnly ? (
                    <span className="private-invitee-panel__count">
            ...
            {selectedProfiles.length > 0 ? (
                <ul className="private-invitee-panel__selected" aria-label={t('invited_friends', 'Invited friends')}>
                    {selectedProfiles.map((friend) => (
                        <li key={friend.id} className="private-invitee-panel__selected-item">
                            <UserAvatar user={friend} alt="" style={{ width: 40, height: 40 }} />
                            <span className="private-invitee-panel__selected-name">
                                {friend.display_name || friend.name || t('user', 'User')}
                            </span>
                            {!readOnly ? (
                                <button
                                    type="button"
                                    className="private-invitee-panel__remove"
                ...
                <p className="private-invitee-panel__empty">{emptyHint}</p>
            )}

            {!readOnly ? (
                <>
                    <div className="private-invitee-panel__search-wrap">
                        <input
                            type="search"
                            className="private-invitee-panel__search"
                            placeholder={t('search_friends')}
                            value={friendSearchQuery}
                            onChange={(e) => setFriendSearchQuery(e.target.value)}
                            autoComplete="off"
                        />
                        <FaSearch className="private-invitee-panel__search-icon" aria-hidden />
                    </div>

                    {friendsLoading || friendSearchLoading ? (
                        <p className="private-invitee-panel__loading">{t('loading')}</p>
                    ) : filteredFriends.length > 0 ? (
                        <div className="private-invitee-panel__grid">
                        ...
                        <p className="private-invitee-panel__empty">{datingListEmptyMessage}</p>
                    )}
                </>
            ) : null}
        </section>
