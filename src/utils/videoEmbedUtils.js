...
/** Twitch usernames: 4-25 chars, letters/numbers/underscore, no leading underscore. */
export function sanitizeTwitchChannel(value) {
    const raw = String(value || '').trim();
    return /^[a-zA-Z0-9][a-zA-Z0-9_]{3,24}$/.test(raw) ? raw.toLowerCase() : '';
}

/** Accepts a bare channel name or a full twitch.tv URL and returns the sanitized channel. */
export function parseTwitchChannelInput(input) {
    const text = String(input || '').trim();
    if (!text) return '';
    const urlMatch = text.match(/(?:^|\/\/)(?:www\.|m\.)?twitch\.tv\/([a-zA-Z0-9_]{4,25})(?:[/?#]|$)/i);
    if (urlMatch?.[1]) return sanitizeTwitchChannel(urlMatch[1]);
    return sanitizeTwitchChannel(text);
}

/**
 * Domains Twitch's embed player is allowed to run on (required `parent` param).
 * Native app WebViews serve from `https://localhost` — Twitch accepts a bare `localhost`.
 */
export function getTwitchEmbedParentDomains() {
    const domains = new Set(['dinebuddies.com', 'www.dinebuddies.com', 'localhost']);
...
/** Live-only embed — no playback controls to sync, Twitch's own CDN keeps everyone near the live edge. */
export function buildTwitchEmbedSrc(channel, { muted = true } = {}) {
    ...
    for (const domain of getTwitchEmbedParentDomains()) {
        params.append('parent', domain);
    }
    return `https://player.twitch.tv/?${params.toString()}`;
}

/** Whether banner has a Twitch channel set. */
export function hasTwitchBannerMedia(banner) {
    return Boolean(banner && sanitizeTwitchChannel(banner.twitchChannel));
}
