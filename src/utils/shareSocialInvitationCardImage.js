...
export async function shareInvitationCardImageWithDeepLink(_element, deepLinkUrl, opts = {}) {
    ...
    if (result === 'downloaded' && deepLinkUrl && !isMobileDevice()) {
        // First-party invite links only — never open arbitrary externals.
        try {
            const { classifyChatLink } = await import('./chatLinkSafety.js');
            const info = classifyChatLink(deepLinkUrl);
