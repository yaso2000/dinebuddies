...
const Home = () => {
  ...
  const filteredInvitations = useMemo(() => {
    ...
    let filtered = safeInvitations.filter((inv) => {
      ...
      // 2. HOME SPECIFIC: EXPIRY LOGIC (hide from feed when due for archive — event end + 24h)
                        ...
                        <div class="compact-popup" dir="auto" style="unicode-bidi:isolate;text-align:start">
                            <div style="position: relative;">
                                ...
                                <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; padding: 4px; background: linear-gradient(to top, rgba(0,0,0,0.6), transparent);">
                                    <span style="background: ${markerColor}; color: white; padding: 2px 6px; border-radius: 4px; font-size: 0.6rem; font-weight: 700; position: absolute; bottom: 4px; left: 4px; unicode-bidi:isolate;" dir="auto">${escapeHtmlText(formatBiDiText(inv.type ? t(`type_${inv.type.toLowerCase().replace(' ', '')}`, inv.type) : t('venue', 'Venue')))}</span>
                                </div>
                            </div>
        ...
        style={{
          ...
          pointerEvents: 'none'
                    ...
                    <div className="footer-info" style={{ pointerEvents: 'auto' }}>
                        <AppText as="h3" style={{ fontSize: '1.5rem', fontWeight: '900', color: 'white', margin: '0 0 6px 0', lineHeight: 1.2, textShadow: '0 2px 4px rgba(0,0,0,0.5)' }}>
                            {restaurant.promoText || 'Special Offer Inside!'}
                        </AppText>
                        {restaurant.tags &&
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                {restaurant.tags.map((tag, i) =>
              <AppText as="span" key={i} style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.8)', background: 'rgba(255,255,255,0.1)', padding: '2px 8px', borderRadius: '4px' }}>
                                        #{tag}
                                    </AppText>
              )}
                            </div>
    ...
    <div className="home-page" style={{ minHeight: '100%', animation: 'fadeIn 0.5s ease-out' }}>
        ...
        zIndex: 50
              ...
              className="filter-select">
                ...
                {/* Row 2: Category filters — always visible */}
                <div
          style={{
            display: 'flex',
            gap: '6px',
          ...
          style={{ borderRadius: isFullscreen ? 0 : undefined, overflow: 'hidden', width: '100%', position: 'relative' }}>
            ...
            style={{ width: '100%', height: '100%', minHeight: isFullscreen ? '100dvh' : undefined, outline: 'none' }} />
                    ...
                    <div className="map-zoom-controls">
                        <button onClick={zoomIn} className="btn-map-control" title={t('zoom_in', { defaultValue: 'Zoom In' })}>                                <AppText as="span" style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>+</AppText>
                        </button>
              ...
              title={isFullscreen ? t('exit_fullscreen', 'Exit Fullscreen') : t('fullscreen', 'Fullscreen')}>
                    ...
                    <div className="map-discovery-badge" style={{ top: 'auto', bottom: '20px', left: '50%', transform: 'translateX(-50%)' }}>
                        ...
                        <AppText as="span">{invitationsWithCoords.length} {t('active_events')}</AppText>
                    </div>
                </div>
            </div>
        ...
        <div
          ...
          style={{
            ...
            gap: '12px'
                        ...
                        <AppText as="span" style={{ fontSize: '0.9rem', fontWeight: '600' }}>{redirectMessage}</AppText>
                        <button
