...
import { AppText } from "../../components/base";const BRAND_TEMPLATES = [
{
...
const BrandKit = ({ onBack, inAppColumn = true }) => {
  ...
  const isPaid = getBusinessSubscriptionAccess(userProfile?.subscriptionTier).isPaid;
  ...
  <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <AppText as="span" style={{ fontSize: '0.8rem', fontWeight: 700, color: '#f1f5f9' }}>{label}</AppText>
                {hint && <AppText as="span" style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.35)' }}>{hint}</AppText>}
            </div>
      ...
      style={{ width: 36, height: 28, border: 'none', borderRadius: 6, cursor: 'pointer', background: 'transparent' }} />
                ...
                <AppText as="span" style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', fontFamily: 'monospace' }}>{value}</AppText>
            </div>
        </div>;
  ...
  return (
    <div className="brand-kit-page" style={{ paddingBottom: isMobile ? '160px' : '1rem' }}>
        ...
        style={{ background: 'var(--hover-overlay)', border: '1px solid var(--border-color)', borderRadius: 10, color: 'var(--text-main)', padding: '6px 12px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7, fontSize: '0.85rem', fontWeight: 600 }}>
                ...
                <AppText as="span" style={{ color: 'var(--text-muted)' }}>/</AppText>
                <AppText as="span" style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--text-main)' }}>Brand Kit</AppText>
