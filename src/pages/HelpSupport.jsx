  ...
  return (
    <div className={!isDashboard ? "page-container" : ""} style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '15px 20px', background: 'var(--bg-card)', borderBottom: '1px solid var(--border-color)', position: 'sticky', top: 0, zIndex: 50 }}>
                    ...
                    <AppText as="h2" style={{ fontSize: '1.1rem', fontWeight: 700, margin: 0, color: 'var(--text-main)', textAlign: 'center', flex: 1 }}>{t('faq.title', { defaultValue: 'Help & Support' })}</AppText>
            ...
            <div style={{ padding: isDashboard ? '1rem 0' : '1.5rem', maxWidth: '800px', margin: '0 auto' }}>
                <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
                    ...
                    <AppText as="h1" style={{ fontSize: '1.5rem', fontWeight: '900', color: 'var(--text-main)', marginBottom: '0.5rem' }}>
                        {t('faq.title', { defaultValue: 'Help & Support' })}
                    </AppText>
                    <AppText as="p" style={{ color: 'var(--text-muted)', fontSize: '0.9rem', maxWidth: '400px', margin: '0 auto' }}>
                ...
                transition: 'all 0.3s ease'
                  ...
                  style={{
                    ...
                    color: 'var(--text-main)'
                                        ...
                                        <AppText as="span" style={{ fontSize: '1rem', fontWeight: '700', paddingRight: '1rem' }}>
                                            {item.q}
                                        </AppText>
                                        <AppText as="span" style={{ color: isOpen ? 'var(--primary)' : 'var(--text-muted)', transition: 'transform 0.3s', transform: isOpen ? 'rotate(180deg)' : 'none' }}>
                                            <FaChevronDown />
                                        </AppText>
                                    </button>
        ...
        <div style={{ textAlign: 'center', padding: '3rem 1rem', color: 'var(--text-muted)' }}>
                        ...
                        <AppText as="p">{t('faq.empty', { defaultValue: 'No questions currently available.' })}</AppText>
                    </div>
        }
            </div>
        </div>);
