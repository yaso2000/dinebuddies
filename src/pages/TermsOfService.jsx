        ...
        <div style={{ padding: '20px', paddingBottom: '100px', color: 'var(--text-main)', maxWidth: '900px', margin: '0 auto', textAlign: i18n.language === 'ar' ? 'right' : 'left', lineHeight: '1.6' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '40px', borderBottom: '1px solid var(--border-color)', paddingBottom: '20px' }}>
                ...
                <div>
                    ...
                    <p style={{ margin: 0, opacity: 0.6 }}>{i18n.language === 'ar' ? 'شروط وقواعد استخدام منصة DineBuddies' : 'Rules and regulations for using the DineBuddies platform'}</p>
                </div>
            </div>

            <section style={{ marginBottom: '40px' }}>
                <h3 style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--primary)' }}>
                    <FaGavel /> {i18n.language === 'ar' ? '1. قبول الاتفاقية' : '1. Acceptance of Agreement'}
                </h3>
                <div style={{ padding: '0 10px' }}>
                    <p>
                        {i18n.language === 'ar'
                            ? 'باستخدامك لمنصة DineBuddies، فإنك تقر بأنك قرأت وفهمت ووافقت على الالتزام بهذه الشروط. هذه الاتفاقية سارية المفعول من لحظة دخولك الأول للمنصة.'
            ...
            <section style={{ marginBottom: '40px' }}>
                <h3 style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--primary)' }}>
                    <FaUserCheck /> {i18n.language === 'ar' ? '2. أهلية المستخدم' : '2. User Eligibility'}
                </h3>
                <div style={{ padding: '0 10px' }}>
                    <p>
                        {i18n.language === 'ar'
                            ? 'يجب أن تكون قد بلغت السن القانوني (18 عاماً فأكثر) لاستخدام خدماتنا. باستخدامك للتطبيق، فإنك تضمن صحة كافة البيانات المقدمة وأنك لا تنتهك أي قوانين محلية في بلدك.'
            ...
            <section style={{ marginBottom: '40px' }}>
                <h3 style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--primary)' }}>
                    <FaBan /> {i18n.language === 'ar' ? '3. السلوك المحظور' : '3. Prohibited Conduct'}
                </h3>
                <div style={{ padding: '0 10px' }}>
                    <p>{i18n.language === 'ar' ? 'يمنع منعاً باتاً القيام بأي مما يلي:' : 'The following is strictly prohibited:'}</p>
                    <ul>
                        <li>{i18n.language === 'ar' ? 'نشر محتوى يحرض على الكراهية، العنف، أو التمييز العنصري.' : 'Posting content that incites hatred, violence, or racial discrimination.'}</li>
                        <li>{i18n.language === 'ar' ? 'انتحال شخصية مستخدم آخر أو شريك تجاري.' : 'Impersonating another user or business partner.'}</li>
                        <li>{i18n.language === 'ar' ? 'استخدام المنصة لأغراض التحرش أو مطاردة المستخدمين.' : 'Using the platform for harassment or stalking purposes.'}</li>
                        <li>{i18n.language === 'ar' ? 'إرسال رسائل سبام (Spam) أو محاولات احتيال مالي.' : 'Sending spam messages or engaging in financial fraud attempts.'}</li>
                    </ul>
                </div>
            </section>
            ...
            <section style={{ marginBottom: '40px' }}>
                <h3 style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--primary)' }}>
                    <FaExclamationTriangle /> {i18n.language === 'ar' ? '4. إخلاء المسؤولية عن التفاعل الفعلي' : '4. Disclaimer: Real-World Interaction'}
                </h3>
                <div style={{ padding: '0 10px' }}>
                    <p>
                        {i18n.language === 'ar'
                            ? 'DineBuddies هي منصة تقنية لتسهيل التواصل والتعارف الجيد فقط. نحن لا نتحمل أي مسؤولية قانونية أو أمنية عما يحدث بين المستخدمين في اللقاءات الحقيقية خارج التطبيق. تقع مسؤولية سلامتك الشخصية وتدقيق هوية من تقابلهم عليك تماماً.'
            ...
            <section style={{ marginBottom: '40px' }}>
                <h3 style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--primary)' }}>
                    <FaBuilding /> {i18n.language === 'ar' ? '5. شروط الشركاء التجاريين (Businesses)' : '5. Terms for Business Partners'}
                </h3>
                <div style={{ padding: '0 10px' }}>
                    <p>
                        {i18n.language === 'ar'
                            ? 'الشركاء التجاريون مسؤولون عن صحة العروض المقدمة وجودة الخدمات في منشآتهم. يحق لـ DineBuddies إنهاء حساب الشريك في حال تكرار الشكاوى أو تقديم معلومات مضللة للمستخدمين.'
            ...
            <div style={{ marginTop: '50px', padding: '25px', background: 'var(--bg-card)', borderRadius: '20px', border: '1px solid var(--border-color)', boxShadow: '0 10px 30px rgba(0,0,0,0.05)' }}>
                <h4 style={{ margin: '0 0 10px 0', color: 'var(--primary)' }}>{i18n.language === 'ar' ? 'تعديل الشروط' : 'Updates to Terms'}</h4>
                <p style={{ margin: 0, fontSize: '0.9rem' }}>
                    {i18n.language === 'ar'
                        ? 'نحتفظ بالحق في تعديل هذه الشروط في أي وقت. سيتم إخطارك بالتغييرات الجوهرية عبر التطبيق أو البريد الإلكتروني.'
