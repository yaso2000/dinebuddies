...
import { resolveSignedInHomePath, isBusinessAccount } from '../utils/accountKind';
...
const VerifyEmail = () => {
  ...
  const goNext = useCallback(() => {
    navigate(resolveSignedInHomePath(currentUser, userProfile), { replace: true });
  ...
  useEffect(() => {
    ...
    if (userProfile && isBusinessAccount(userProfile)) {
      navigate(resolveSignedInHomePath(currentUser, userProfile), { replace: true });
      return;
    }
      ...
      style={{
        ...
        boxShadow: '0 16px 40px rgba(0,0,0,0.25)'
                ...
                <div style={{ textAlign: 'center', marginBottom: '1.25rem' }}>
                    ...
                    <AppText as="h1" style={{ fontSize: '1.35rem', fontWeight: 900, color: 'var(--text-main)', margin: '0 0 0.35rem' }}>
                        {t('verify_email_title', 'Activate your account')}
                    </AppText>
                    <AppText as="p" style={{ color: 'var(--text-muted)', fontSize: '0.9rem', margin: 0, lineHeight: 1.5 }}>
                        {t('verify_email_subtitle', 'We sent a verification link to:')}
                    </AppText>
                    <AppText as="p" style={{ color: 'var(--primary)', fontWeight: 700, fontSize: '0.95rem', marginTop: 8, wordBreak: 'break-all' }}>
                        {email}
                    </AppText>
                </div>
          ...
          lineHeight: 1.55
                    ...
                    {t('verify_email_hint', 'Open the email and tap “Verify Account”. On iPhone, the link opens in Safari — after verifying, return to the app and tap “I’ve verified”.')}
                </div>
