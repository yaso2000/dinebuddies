...
const BusinessManagement = () => {
    ...
    const filterBusinesses = () => {
        ...
        if (searchQuery) {
            ...
            filtered = filtered.filter(b =>
                ...
                b.businessInfo?.businessName?.toLowerCase().includes(q)
            );
        }
    ...
    const handleBanBusiness = async (businessId, currentStatus) => {
        ...
        try {
            ...
            setBusinesses(businesses.map(b =>
                b.id === businessId ? { ...b, banned: !currentStatus } : b
            ));
    ...
    const stats = {
        ...
        active: businesses.filter(b => !b.banned && b.businessInfo?.published).length,
        banned: businesses.filter(b => b.banned).length,
        unpublished: businesses.filter(b => !b.businessInfo?.published).length
    };
    ...
    return (
        <div>
            ...
            <div className="admin-page-header admin-mb-4">
                ...
                <p className="admin-page-subtitle">Manage business accounts</p>
            </div>
                ...
                <div className="admin-card">
                    <div className="admin-empty">
                        ...
                        <p className="admin-empty-text">
                            {searchQuery ? 'Try a different search term' : 'No businesses yet'}
                        </p>
                    </div>
                </div>
            ) : (
                                ...
                                <tr key={business.id}>
                                    ...
                                    <td>
                                        <div className="admin-flex admin-gap-1" style={{ alignItems: 'center' }}>
                                            ...
                                            <span style={{ fontSize: '0.875rem', color: '#94a3b8' }}>
                                                {business.businessInfo?.phone || 'N/A'}
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <div className="admin-flex admin-gap-1" style={{ alignItems: 'center' }}>
                                            ...
                                            <span style={{ fontSize: '0.875rem', color: '#94a3b8' }}>
                                                {business.businessInfo?.location || 'N/A'}
                                            </span>
                                        </div>
                                    </td>
                        ...
                        <div style={{ display: 'grid', gap: '1rem' }}>
                            <div>
                                ...
                                <div style={{ color: '#ffffff', fontWeight: '600' }}>
                                    {selectedBusiness.businessInfo?.businessName || 'N/A'}
                                </div>
                            </div>
                            <div>
                                <label className="admin-label">Category</label>
                                <div style={{ color: '#ffffff' }}>
                                    {selectedBusiness.businessInfo?.category || 'N/A'}
                                </div>
                            </div>
                            <div>
                                <label className="admin-label">Description</label>
                                <div style={{ color: '#ffffff' }}>
                                    {selectedBusiness.businessInfo?.description || 'N/A'}
                                </div>
                            </div>
                            <div>
                                <label className="admin-label">Owner Name</label>
                                <div style={{ color: '#ffffff' }}>
                                    {selectedBusiness.displayName || 'N/A'}
                                </div>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <div style={{ color: '#ffffff' }}>
                                    {selectedBusiness.businessInfo?.phone || 'N/A'}
                                </div>
                            </div>
                            <div>
                                <label className="admin-label">Location</label>
                                <div style={{ color: '#ffffff' }}>
                                    {selectedBusiness.businessInfo?.location || 'N/A'}
                                </div>
                            </div>
                            <div>
                                <label className="admin-label">Joined</label>
                                <div style={{ color: '#ffffff' }}>
                                    {selectedBusiness.createdAt?.toDate?.()?.toLocaleString() || 'N/A'}
                                </div>
                            </div>
                        </div>
