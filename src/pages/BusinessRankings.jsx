...
export default function BusinessRankings() {
  ...
  useEffect(() => {
    ...
    if (scope === 'city') currentLimit = 10;else
    if (scope === 'country') currentLimit = 50;
  ...
  return (
    <div style={{ padding: 24, maxWidth: '100%', width: '100%', margin: 0, boxSizing: 'border-box' }}>
            <AppText as="h1" style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: 8 }}>
                {t('rankings_title', 'Business Rankings')}
            </AppText>
            <AppText as="p" style={{ fontSize: '0.9rem', marginBottom: 20 }}>
        ...
        <button
          ...
          style={{
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                <AppText as="span" style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-main)', background: 'var(--bg-elevated)', padding: '6px 12px', borderRadius: 10, border: '1px solid var(--border)' }}>
                    {scopeLabel || t('rankings_global', 'Global')}
                </AppText>
                <AppText as="span" style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                    {totalInScope} {t('rankings_businesses', 'businesses')}
                </AppText>
                <AppText as="span" style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', opacity: 0.85 }}>
        ...
        style={{
          ...
          minHeight: 200
            ...
            style={{
              ...
              <AppText as="span" style={{ position: 'absolute', bottom: -2, right: -2, width: 22, height: 22, borderRadius: '50%', background: 'var(--bg-card)', border: '2px solid #C0C0C0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', lineHeight: 1, boxSizing: 'border-box' }} title={ranked[1].business.businessPublic.country}>{getCountryFlag(ranked[1].business.businessPublic.country)}</AppText> :
              null}
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 6 }}>
                                <AppText as="span" style={{ fontSize: '1.25rem' }}>🥈</AppText>
                                <AppText as="span" style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: 600 }}>{Math.round(ranked[1].business.rankingScore ?? 0)} {t('rankings_points', 'pts')}</AppText>
                            </div>
            ...
            style={{
              ...
              <AppText as="span" style={{ position: 'absolute', bottom: -2, right: -2, width: 24, height: 24, borderRadius: '50%', background: 'var(--bg-card)', border: '2px solid #D4AF37', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', lineHeight: 1, boxSizing: 'border-box' }} title={ranked[0].business.businessPublic.country}>{getCountryFlag(ranked[0].business.businessPublic.country)}</AppText> :
              null}
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 6 }}>
                                <AppText as="span" style={{ fontSize: '1.4rem' }}>🥇</AppText>
                                <AppText as="span" style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 600 }}>{Math.round(ranked[0].business.rankingScore ?? 0)} {t('rankings_points', 'pts')}</AppText>
                            </div>
                            <div style={{ width: '100%', minHeight: 32, maxHeight: 56, overflow: 'hidden', textAlign: 'center', padding: '0 4px', boxSizing: 'border-box', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            style={{
              ...
              <AppText as="span" style={{ position: 'absolute', bottom: -2, right: -2, width: 20, height: 20, borderRadius: '50%', background: 'var(--bg-card)', border: '2px solid #CD7F32', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.65rem', lineHeight: 1, boxSizing: 'border-box' }} title={ranked[2].business.businessPublic.country}>{getCountryFlag(ranked[2].business.businessPublic.country)}</AppText> :
              null}
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 6 }}>
                                <AppText as="span" style={{ fontSize: '1.25rem' }}>🥉</AppText>
                                <AppText as="span" style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: 600 }}>{Math.round(ranked[2].business.rankingScore ?? 0)} {t('rankings_points', 'pts')}</AppText>
                            </div>
                            <div style={{ width: '100%', minHeight: 32, maxHeight: 56, overflow: 'hidden', textAlign: 'center', padding: '0 4px', boxSizing: 'border-box', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <li
          ...
          style={{
            ...
            cursor: 'pointer'
                        ...
                        <AppText as="span" style={{ fontWeight: 800, color: '#6366f1', minWidth: 28, flexShrink: 0 }}>#{rank}</AppText>
                        <img
            ...
            onError={(e) => {e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(business.name)}`;}} />
                        ...
                        <AppText as="span" style={{ fontWeight: 600, flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', lineHeight: 1.25, wordBreak: 'normal', fontSize: 'clamp(0.7rem, 1.5vw + 0.55rem, 0.95rem)' }} title={business.name}>{getDisplayName(business.name)}{scope === 'global' && business.businessPublic?.country ? ` ${getCountryFlag(business.businessPublic.country)}` : ''}</AppText>
                        <AppText as="span" style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', flexShrink: 0 }}>{Math.round(business.rankingScore ?? 0)} {t('rankings_points', 'pts')}</AppText>
                    </li>
        )}
            </ul>
      ...
      <div style={{ color: 'var(--text-secondary)', textAlign: 'center', padding: 24 }}>
                    <AppText as="p">{scope === 'global' ? t('rankings_no_businesses', 'No businesses in this scope.') : t('rankings_no_in_scope', 'No businesses in this area. Try Global.')}</AppText>
                    {errorMsg &&
        <AppText as="p" style={{ marginTop: 12, color: '#ef4444', fontSize: '0.85rem', fontWeight: 'bold' }}>
