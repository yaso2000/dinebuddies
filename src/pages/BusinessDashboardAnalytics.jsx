  ...
  return (
    <div className="my-community-stat-card ui-card" style={{ textAlign: 'start', padding: '14px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                <AppText as="span" style={{ color: 'var(--primary)', fontSize: '1.1rem' }}>{icon}</AppText>
                <AppText as="span" style={{ fontWeight: 700, fontSize: '0.9rem', color: 'var(--text-main)' }}>{label}</AppText>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: '12px' }}>
                <AppText as="span" style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                    {count} {unitLabel} · {multiplierLabel}
                </AppText>
                <AppText as="span" style={{ fontWeight: 800, fontSize: '1.1rem', color: 'var(--primary)' }}>+{points}</AppText>
            </div>
        </div>);
      ...
      <BusinessDashboardShell
        ...
        icon={<FaChartLine />}>
                ...
                <div className="my-community-stat-card ui-card" style={{ textAlign: 'center', padding: '2rem 1.25rem' }}>
                    ...
                    <AppText as="p" style={{ margin: '0 0 16px', fontSize: '0.9rem', color: 'var(--text-muted)', lineHeight: 1.5 }}>
                        {t(
    ...
    <BusinessDashboardShell
      ...
      <>
                    <div className="my-community-stat-card ui-card" style={{ textAlign: 'center', padding: '2rem 1rem', marginBottom: '1rem' }}>
                        <FaChartLine style={{ fontSize: '2rem', color: 'var(--primary)', marginBottom: '12px' }} />
                        <AppText as="h4" style={{ fontSize: '2.5rem', fontWeight: 800, margin: '8px 0' }}>{breakdown?.total ?? 0}</AppText>
                        <AppText as="p" style={{ fontSize: '0.9rem', color: 'var(--text-muted)', margin: 0 }}>
                            {t('biz_points_total', 'Total ranking points')}
                        </AppText>
                    </div>

                    <AppText as="p" className="my-community-empty ui-prompt__desc" style={{ marginBottom: '1rem' }}>
