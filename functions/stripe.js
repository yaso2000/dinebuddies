...
exports.createCheckoutSession = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const metadata = {
            ...
            planId: plan.id,
            planName: plan.name,
            checkoutType: plan.mode === 'subscription' ? 'subscription' : plan.purchaseType,
        };
        if (plan.tier) {
            metadata.tier = plan.tier;
            metadata.currentPlan = plan.currentPlan;
            metadata.weeklyPrivateQuota = String(plan.weeklyPrivateQuota);
        }
        if (plan.creditsField) {
            metadata.creditsField = plan.creditsField;
