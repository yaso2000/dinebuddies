...
async function assertAdminContext(context) {
    ...
    const isSuperOwner = isConfiguredSuperOwner || context.auth.token.superOwner === true;
...
exports.grantAdminRole = functions.https.onCall(async (data, context) => {
    ...
    const requesterUid = context.auth.uid;
    const requesterEmail = (context.auth.token.email || '').toLowerCase();
    const requesterIsSuperOwner =
        SUPER_OWNER_UIDS.includes(requesterUid) ||
        SUPER_OWNER_EMAILS.includes(requesterEmail) ||
        context.auth.token.superOwner === true;
    if (!requesterIsSuperOwner) {
