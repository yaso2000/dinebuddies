...
async function assertAdminContext(context) {
    ...
    const isConfiguredSuperOwner = SUPER_OWNER_UIDS.includes(requesterUid) || SUPER_OWNER_EMAILS.includes(requesterEmail);
    const isSuperOwner = context.auth.token.superOwner === true || isConfiguredSuperOwner;
...
exports.grantAdminRole = functions.https.onCall(async (data, context) => {
    ...
    const targetIsConfiguredSuperOwner = SUPER_OWNER_UIDS.includes(targetUid) || SUPER_OWNER_EMAILS.includes(targetEmail);
