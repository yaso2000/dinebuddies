...
const { getCheckoutPlanByPriceId, getVerifiedCheckoutPlan } = require('./paymentPlans');
...
async function handleCheckoutComplete(session) {
    ...
    const plan = getVerifiedCheckoutPlan(requestedPlanId, actualPriceId, session.mode);
    if (!plan) {
        throw new Error(`Checkout plan, price, or mode mismatch for session ${session.id}`);
    }
...
async function handleSubscriptionUpdate(subscription) {
    ...
    if (!plan || plan.kind !== 'subscription') {
        console.error('Unknown subscription price:', actualPriceId);
        return;
    }
