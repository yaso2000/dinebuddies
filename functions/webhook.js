...
async function handleSubscriptionUpdate(subscription) {
    ...
    const rawPlanId = subscription.metadata?.planId || usersSnapshot.docs[0].data().currentPlan;
    const checkoutProduct = findByPlanId(rawPlanId);
    const planId = checkoutProduct?.planId || rawPlanId;
    const weeklyQuota = Number.isFinite(checkoutProduct?.weeklyQuota)
        ? checkoutProduct.weeklyQuota
        : getQuotaForPlan(planId);
    const tier = checkoutProduct?.tier || getTierForPlan(planId);
