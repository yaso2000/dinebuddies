...
- **2.1 + 2.10–2.17** Architectural, **multi-week**, do incrementally (app stays deployable):
  - **2.1 — ✅ DONE + DEPLOYED 2026-09-18.** `users/{uid}` rule locked to `allow get: isOwner || isAdminOrPanelStaff` + list admin-only(≤200); broad signed-in read/list removed. All client cross-user reads migrated to the `public_profiles` projection (10 slices: directory, ranking, invitation/community cards, settings lists, chat gates via a `users.followers[]` reverse index, follow flow, block/mute [server-enforced by createOrGetConversation], chat participants, followers-list, avatar hydration). Remaining `users` reads are own-doc or best-effort public-first (catch→public). New projection fields (profileLikes, communityMemberCount, availableForPrivateInvite, showFriends, isSystemAccount, coarse geo, card fields) all have safe defaults. Verified live: Partners directory loads fully, UserProfile falls back to public projection on denial. Deployed in order functions→web→rules. Backfill of followers[] + public_profiles was run once (the SA key used was then revoked+deleted).
