...
const pt = JSON.parse(fs.readFileSync(path.join(root, 'src/locales/pt.json'), 'utf8'));
...
for (const key of Object.keys(en).sort()) {
    ...
    if (pt[key] === en[key] && !shouldKeepEnglish(key, en[key])) {
        missing[key] = en[key];
    }
}

const outPath = path.join(__dirname, '_pt-missing-keys.json');
