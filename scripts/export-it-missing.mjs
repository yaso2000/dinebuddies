...
const it = JSON.parse(fs.readFileSync(path.join(root, 'src/locales/it.json'), 'utf8'));
...
for (const key of Object.keys(en).sort()) {
    ...
    if (it[key] === en[key] && !shouldKeepEnglish(key, en[key])) {
        missing[key] = en[key];
    }
}

const outPath = path.join(__dirname, '_it-missing-keys.json');
