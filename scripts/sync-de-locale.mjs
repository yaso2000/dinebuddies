/**
 * Offline German locale sync — merges manual batches into de.json.
 * Usage: node scripts/sync-de-locale.mjs
 */
import fs from 'fs';
...
const deExisting = JSON.parse(fs.readFileSync(path.join(root, 'src/locales/de.json'), 'utf8'));

function loadManualBatches() {
    const out = {};
    for (const name of fs.readdirSync(__dirname)) {
        if (!name.startsWith('locale-de-') || !name.endsWith('.json')) continue;
...
fs.writeFileSync(path.join(root, 'src/locales/de.json'), `${JSON.stringify(sorted, null, 2)}\n`, 'utf8');
...
console.log(JSON.stringify({
    deKeys: Object.keys(sorted).length,
