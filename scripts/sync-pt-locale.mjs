/**
 * Offline Portuguese locale sync — merges manual batches into pt.json.
 * Usage: node scripts/sync-pt-locale.mjs
 */
import fs from 'fs';
...
const ptExisting = JSON.parse(fs.readFileSync(path.join(root, 'src/locales/pt.json'), 'utf8'));

function loadManualBatches() {
    const out = {};
    for (const name of fs.readdirSync(__dirname)) {
        if (!name.startsWith('locale-pt-') || !name.endsWith('.json')) continue;
...
fs.writeFileSync(path.join(root, 'src/locales/pt.json'), `${JSON.stringify(sorted, null, 2)}\n`, 'utf8');
...
console.log(JSON.stringify({
    ptKeys: Object.keys(sorted).length,
