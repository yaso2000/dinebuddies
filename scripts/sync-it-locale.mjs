/**
 * Offline Italian locale sync — merges manual batches into it.json.
 * Usage: node scripts/sync-it-locale.mjs
 */
import fs from 'fs';
...
const itExisting = JSON.parse(fs.readFileSync(path.join(root, 'src/locales/it.json'), 'utf8'));

function loadManualBatches() {
    const out = {};
    for (const name of fs.readdirSync(__dirname)) {
        if (!name.startsWith('locale-it-') || !name.endsWith('.json')) continue;
...
fs.writeFileSync(path.join(root, 'src/locales/it.json'), `${JSON.stringify(sorted, null, 2)}\n`, 'utf8');
...
console.log(JSON.stringify({
    itKeys: Object.keys(sorted).length,
