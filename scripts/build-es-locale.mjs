/**
 * Build es.json from en.json — preserves existing Spanish translations.
 * Usage: node scripts/build-es-locale.mjs [--dry-run] [--limit=N] [--skip-auto]
 */
import fs from 'fs';
...
let es = JSON.parse(fs.readFileSync(path.join(root, 'src/locales/es.json'), 'utf8'));
...
async function translateEnToEs(text) {
    ...
    const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=es&dt=t&q=${encodeURIComponent(safe)}`;
...
function loadManualBatches() {
    ...
    for (const name of fs.readdirSync(__dirname)) {
        if (!name.startsWith('locale-es-') || !name.endsWith('.json')) continue;
...
async function main() {
    ...
    for (const key of keys) {
        ...
        try {
            result[key] = await translateEnToEs(enVal);
            autoCount += 1;
            if (autoCount % 25 === 0) {
    ...
    if (!dryRun) {
        fs.writeFileSync(path.join(root, 'src/locales/es.json'), `${JSON.stringify(sorted, null, 2)}\n`, 'utf8');
    }
}
