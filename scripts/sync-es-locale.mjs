/**
 * Offline Spanish locale sync — merges manual batches into es.json.
 * Usage: node scripts/sync-es-locale.mjs
 */
import fs from 'fs';
...
function loadManualBatches() {
    ...
    for (const name of fs.readdirSync(__dirname)) {
        ...
        const chunk = JSON.parse(fs.readFileSync(path.join(__dirname, name), 'utf8'));
...
console.log(JSON.stringify({
    esKeys: Object.keys(sorted).length,
