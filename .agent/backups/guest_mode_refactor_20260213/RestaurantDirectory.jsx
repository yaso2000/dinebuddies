...
const RestaurantCard = React.memo(({ res, onViewMembers }) => {
    ...
    useEffect(() => {
        if (res.reviews && Array.isArray(res.reviews)) {
            ...
            const total = res.reviews.reduce((acc, curr) => acc + curr.rating, 0);
            setAverageRating(res.reviews.length > 0 ? total / res.reviews.length : 0);
        }
    }, [res]);
    ...
    return (
        ...
        >
                ...
                <div style={{ position: 'absolute', top: '16px', left: '16px', right: '16px', display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                        <button
                            ...
                            onMouseEnter={(e) => {
                                e.currentTarget.style.background = 'rgba(55, 7, 74, 0.5)';
                                e.currentTarget.style.transform = 'scale(1.1)';
                            }}
                            onMouseLeave={(e) => {
                                e.currentTarget.style.background = 'rgba(55, 7, 74, 0.3)';
            ...
            <div style={{ padding: '20px', background: 'transparent' }}>
                {/* Title & Info Section - Moved from Overlay */}
                <div style={{ marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                        <h2 style={{ fontSize: '1.5rem', fontWeight: '900', color: 'var(--text-main)', marginBottom: '5px' }}>{res.name}</h2>
                        <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '5px' }}>
                            <FaMapMarkedAlt size={12} style={{ color: 'var(--primary)' }} /> {res.location}
                        </p>
                    </div>
                    {reviewCount > 0 && (
                        <div
                        ...
                        >
                            <span style={{ fontSize: '0.9rem', color: 'var(--luxury-gold)' }}>⭐</span>
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        {communityMembers && communityMembers.length > 0 ? (
                            ...
                            >
                                ...
                                <span style={{ marginLeft: '8px', fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: '600' }}>
                    ...
                    {isOwner ? (
                        ...
                        <button
                            ...
                            onMouseEnter={(e) => {
                                if (!isJoined) e.currentTarget.style.background = 'var(--primary-hover)';
                                e.currentTarget.style.transform = 'scale(1.05)';
                            }}
                            onMouseLeave={(e) => {
                                if (!isJoined) e.currentTarget.style.background = 'var(--primary)';
                                e.currentTarget.style.transform = 'scale(1)';
                            }}
                        >
        ...
        <div className="directory-page" style={{ paddingBottom: '100px', minHeight: '100vh' }}>
            ...
            {context?.currentUser?.id === 'guest' && (
                <div style={{
                    ...
                    <p style={{ color: 'white', margin: '0 0 10px 0', fontSize: '0.9rem', fontWeight: '600' }}>
                        {t('guest_directory_message', { defaultValue: 'Sign in to join communities and interact with partners!' })}
                    </p>
                    <button
